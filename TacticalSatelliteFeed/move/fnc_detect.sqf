	private 
	[
		"_climber","_posa","_posb","_int","_int2","_int3","_int5","_intcount","_anm","_dPos","_pos","_toppos","_goodZ","_blocked",
		"_top","_poses","_obj","_succ","_testpos","_posWT","_ppWT","_tpWT","_dst","_toptest","_postopos",
		"_a","_b","_c","_d","_e","_f","_respos","_min","_max","_avZ","_bone","_wide","_rpos","_mtw","_liF","_condPars"
	];
	_climber = _this;

	if (_climber isEqualTo []) then
	{
		_climber = player;
	};
		...
		{ 
			...
			if (_intcount == 0) then
			{
				...
				if (_anm != "" && str _dpos != "[0,0,0]") then
				{
					...
					//EM_climbing = true;
					//_climber setVariable ["TSF_unitIsClimbing",true,false];

					//[ (format ["EH_em_drop%1",([_climber] call TSF_fnc_getUnitNumber)]) , {animationState (_condpars select 0) == (_condpars select 1)}, [_climber, _anm], "TSF_fnc_exec_drop", [_dpos, _climber], true, "TSF_fnc_finish_drop", [_climber], 0] call babe_core_fnc_addEH;
					_result = [_dpos,_anm];
