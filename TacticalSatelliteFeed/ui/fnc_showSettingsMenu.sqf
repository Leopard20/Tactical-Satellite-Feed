...
[] spawn {
	...
	["TSF_showSettings_EH", "onEachFrame",  
	{
		params ["_ctrls", "_xFinal", "_display"];
		_ctrl = _display displayCtrl 1655;
		_xp = (ctrlPosition _ctrl) select 0;
		_xp = _xp - 0.015 * safezoneW;
		if !(TSF_camActive && TSF_showSettingsMenu && _xFinal<=_xp) exitWith {
			["TSF_showSettings_EH", "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
			{
				_ctrl = _display displayCtrl _x;
				_yp = (ctrlPosition _ctrl) select 1;
